...
# --------------------------------------------------------------------------- tables
def table_cells(agg, rows, methods, decimals=None):
    """Mean +- s.e. strings for a list of table rows; the best method and every method within one s.e. of it in bold.

    Each row is a dict with keys dataset, sweep, panel, kind, level, n_0_level, metric,
    higher_better, label; `agg` maps dataset -> aggregate(load(dataset)). Human-only
    cells where the human-only MLE existed in fewer than half the splits get a dagger.
    Returns (DataFrame of strings indexed by label, DataFrame of means).
    """
    out, means = [], []
    for r in rows:
        a = agg[r["dataset"]]
        g = a[(a.sweep == r["sweep"]) & (a.panel == r["panel"]) & (a.kind == r["kind"]) & np.isclose(a.level, r["level"]) & (a.n_0_level == r.get("n_0_level", -1))]
        m = r["metric"]
        vals = {mm: (float(g.loc[g.method == mm, f"{m}_mean"].iloc[0]) if (g.method == mm).any() else np.nan) for mm in methods}
        ses = {mm: (float(g.loc[g.method == mm, f"{m}_se"].iloc[0]) if (g.method == mm).any() else np.nan) for mm in methods}
        finite = {k: v for k, v in vals.items() if np.isfinite(v)}
        best = (max if r["higher_better"] else min)(finite, key=finite.get) if finite else None
        # bold every method within one standard error (of the best) of the best value
        tol = ses[best] if (best is not None and np.isfinite(ses[best])) else 0.0
        bold = {k for k, v in finite.items() if abs(v - finite[best]) <= tol} if best is not None else set()
        d = r.get("decimals", decimals or (3 if m == "ref_kendall" else 4))
        cells = {}
        for mm in methods:
            if not np.isfinite(vals[mm]):
                cells[mm] = "--"
                continue
            s = f"{vals[mm]:.{d}f}{{\\scriptsize$\\pm${ses[mm]:.{d}f}}}" if np.isfinite(ses[mm]) else f"{vals[mm]:.{d}f}"
            if mm == "human_only" and (g.method == "human_only").any() and float(g.loc[g.method == "human_only", "ho_exists_frac"].iloc[0]) < 0.5:
                s += "$^\\dagger$"
            if mm in bold:
                s = f"\\textbf{{{s}}}"
            cells[mm] = s
        out.append(dict(label=r["label"], **cells))
        means.append(dict(label=r["label"], **vals))
    return pd.DataFrame(out).set_index("label"), pd.DataFrame(means).set_index("label")


def to_booktabs(blocks, methods, labels, path, caption, label, column_width=None, note=None, size="\\footnotesize", resize=True):
    """Write a booktabs table with block headers.

    `blocks` is a list of (block_title, DataFrame of strings from table_cells). With `resize`
    the tabular is scaled to the text width.
    """
    lines = ["\\begin{table}[!t]", "\\centering", size, "\\setlength{\\tabcolsep}{3pt}", f"\\caption{{{caption}}}", f"\\label{{{label}}}"]
    if resize:
        lines.append("\\resizebox{\\textwidth}{!}{%")
    lines.append("\\begin{tabular}{l" + "c" * len(methods) + "}")
    lines.append("\\toprule")
    lines.append(" & " + " & ".join(labels[m] for m in methods) + " \\\\")
    lines.append("\\midrule")
    for title, df in blocks:
        lines.append(f"\\multicolumn{{{len(methods) + 1}}}{{l}}{{\\emph{{{title}}}}} \\\\")
        for lab, row in df.iterrows():
            lines.append(f"\\quad {lab} & " + " & ".join(row[m] for m in methods) + " \\\\")
        lines.append("\\midrule")
    lines[-1] = "\\bottomrule"
    lines.append("\\end{tabular}" + ("}" if resize else ""))
    if note:
