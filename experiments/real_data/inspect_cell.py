...
def build(dataset, sweep, panel_name, kind, level, n_0_req, seed, cfg=None):
    """Return the arrays of one cell: N, K, K_real, A, pairs, test_recs, floor, r_llm, lam_grid, n_L, n_0."""
    ...
    if sweep == "order":
        ...
        if sweep == "noise_scarce":
            n_0 = int(rb.per_dataset(swcfg.get("n_0_fixed", n_0), dataset))
    elif sweep == "budget":
        n_0 = int(level)
    ...
    cal = rb.draw_budget(hum_train, n_0, rng_budget)
    N = panel["N"]
    A = rb.llm_arrays(llm, N, K)
    pairs = rb.human_pairs(cal)
    test_recs = rb.human_records(hum_test)
    n_L, n_0a = int(len(llm)), int(len(cal))
    mults = scfg.get("lambda_multipliers")
    lam_grid = [float(mm) * n_L / n_0a for mm in mults] if mults else None
    ...
    return dict(N=N, K=K, K_real=K_real, K_dropped=K_dropped, A=A, pairs=pairs, test_recs=test_recs, floor=floor, r_llm=r_llm, lam_grid=lam_grid, n_L=n_L, n_0=n_0a)
...
def report(c):
    ...
    print(f"DIAL: selected lam={sel['lam']:.3g} (rel {sel['lam'] * c['n_0'] / c['n_L'] if np.isfinite(sel['lam']) else 'inf'}), dropped={sel['dropped']}, human_only_exists={sel['human_only_exists']}")
    cands = dict(sel["candidates"])
    for p in sel["gacv_path"]:
        f = cands[p["lam"]]
        fi = f.get("fit_info", {}) or {}
        ex = heldout_log_loss(f["s_H"], c["test_recs"]) - c["floor"]
        rel = (p["lam"] * c["n_0"] / c["n_L"]) if np.isfinite(p["lam"]) else float("inf")
...
def main(argv=None):
    ...
    p.add_argument("--n0", type=int, default=None); p.add_argument("--seed", type=int, default=0)
    a = p.parse_args(argv)
    c = build(a.dataset, a.sweep, a.panel, a.kind, a.level, a.n0, a.seed)
    print(f"{a.dataset} {a.sweep} {a.panel} {a.kind} {a.level:g} seed {a.seed}: N={c['N']} K={c['K']} (dropped {c['K_dropped']}) n_L={c['n_L']} n_0={c['n_0']}")
    report(c)
