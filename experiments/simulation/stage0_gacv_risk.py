...
Prop. gacv-additional: max_lambda |GACV(lambda) - R_0(s_hat_lambda)| = O_p(n_H^{-1/2}), and the
...
No leave-one-out refits are needed, so a fine lambda grid and several n_H are cheap.
"""
...
def _endpoint_gacv(X, s_hat, i_obs, j_obs, z_obs):
    """GACV for a fixed-design logistic fit with design rows X (n_H, d): chart = its coefficients."""
    ...
    return ell + float(np.trace(np.linalg.pinv(H) @ J)) / (n_H - 1)


def one(args):
    cfg, n_L, n_H, seed, n_grid = args
    N, K, r = CFG[cfg]
    ...
    grid = [float(m * n_L / n_H) for m in np.geomspace(0.02, 50.0, n_grid)]
    ...
    return dict(
        cfg=cfg, seed=seed, n_H=n_H, n_L=n_L, R0=R0, rows=rows, seconds=time.perf_counter() - t0,
...
if __name__ == "__main__":
    ...
    with ProcessPoolExecutor(2) as ex:
        for n_H in cells:
            recs = list(ex.map(one, [(cfg, n_L, n_H, s, n_grid) for s in range(seeds)]))
            out[str(n_H)] = recs
            ...
            print(f"n_H={n_H:5d}: max|GACV-R0(s_hat)| all {gap.mean():.4f}  excl. lam=0 {gapr.mean():.4f} (sd {gapr.std():.4f})"
    ...
    print(f"log-log slope of max|GACV-R0| (excl. lam=0) vs n_H: {np.polyfit(np.log(ns), np.log(g), 1)[0]:.2f}  (Prop: -1/2)")
