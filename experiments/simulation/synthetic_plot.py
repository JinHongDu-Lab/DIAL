"""Aggregate results/simulation/<config>/rows.jsonl into per-cell summaries (mean, MC s.e., selection rates).

Aggregation is over all stored seeds (append more with synthetic.py). Cells whose staged or
...
def load(cfg, base="."):
    """Load results/simulation/<cfg>/rows.jsonl (relative to `base`) and flag irregular replications."""
    rows = [json.loads(l) for l in open(Path(base) / "results" / "simulation" / cfg / "rows.jsonl")]
...
if __name__ == "__main__":
    ...
    outdir = Path(f"results/simulation/{a.config}")
